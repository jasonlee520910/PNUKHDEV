<?php
	...
	$pillodPillcapa=($_GET["pillodPillcapa"])?$_GET["pillodPillcapa"]:0;//처방량 10g
	$pillodQty=($_GET["pillodQty"])?$_GET["pillodQty"]:0;//처방량 1000개 
	$pilltotalcapa=($_GET["pilltotalcapa"])?$_GET["pilltotalcapa"]:0;//처방량 10g*1000개=10000

	if($apiCode!="goodsgpill"){$json["resultMessage"]="API(apiCode) ERROR1";$apiCode="goodsgpill";}
	...
	else
	{
		...
		$json=array(
			...
			"pillOrderList"=>$pillOrderList,//제환순서 
		...
		if($seq == 'write')
		{
			if(!isEmpty($rc_seq))
			{
				...
				$od_pillcapa=$mrlist["odPillcapa"];
				if(!isEmpty($pillodPillcapa))
				{
					$od_pillcapa=$pillodPillcapa;
				}
			}
		}
		else
		...
		$json["dcWater"]=$mrlist["dcWater"];
		$json["pilllist"]=$mrlist["pilllist"];//orderpill 저장할 데이터들..
		$json["odPillcapa"]=$od_pillcapa;//경옥고스틱파우치에 등록된 제품용량을 불러온다.(즉, 부자재에 등록된 제품용량이 있는것에서 첫번째인것 가져오기 )
