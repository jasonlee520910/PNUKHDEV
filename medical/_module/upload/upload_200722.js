	...
	$('#frm').ajaxForm({
		...
		complete:function(xhr){
			...
			else
			{
				...
				/*
				if(obj["message"] == "FILE_UPLOAD_FAIL")
					alertsign('warning','파일업로드에 실패했습니다.','top',1500);//파일업로드에 실패했습니다.
				else if(obj["message"] == "FILE_UPLOAD_ERR01")
					alertsign('warning','첨부파일 사이즈는 5MB 이내로 등록 가능합니다.','top',1500);//첨부파일 사이즈는 5MB 이내로 등록 가능합니다.
				else if(obj["message"] == "FILE_UPLOAD_ERR02")
					alertsign('warning','허용된 파일형식이 아닙니다.','top',1500);//허용된 파일형식이 아닙니다.
				else if(obj["message"] == "FILE_UPLOAD_ERR04")
					alertsign('warning','도메인 관리자에게 문의 바랍니다.','top',1500);//도메인 관리자에게 문의 바랍니다.
				else 
					alertsign('warning','파일 오류입니다.','top',1500);//파일 오류입니다.
				*/
			}
		}
	});
