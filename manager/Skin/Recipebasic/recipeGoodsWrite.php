...
<input type="hidden" name="apiCode" class="reqdata" value="recipegoodsupdate">
<input type="hidden" name="returnData" class="reqdata" value="<?=$root?>/Skin/Recipebasic/recipeGoodsList.php">
...
<script>
	...
	function viewlayerPopup(obj)
	{
		...
		var data = "&page=1&psize=5&block=10";
		...
		console.log("=========>>>> viewlayerPopup url = " + data);
	...
	function recipegoodsupdate()
	{
		if(necdata()=="Y") //필수조건 체크
		{
			...
			callapi("POST","recipe","recipegoodsupdate",jsondata);
		}
	}
    function makepage(json)
    {
		...
		if(obj["apiCode"] == 'recipegoodsdesc')
		{
			...
			if(!isEmpty(seq))
			{
				...
				$("input[name=rcTitle]").attr("readonly",true); ///처방명은 수정못하게 처리
			}
			else
			{
				$("input[name=rcTitle]").val(obj["rcTitle"]);
			...
			if(!isEmpty(obj["seq"]) && (obj["rcStatus"]=="F"))
			{
				if(Auth=="true")
				{
					btnHtml='<a href="javascript:;" onclick="recipegoodsupdate();" class="bdp-btn"><span><?=$txtdt["1070"]?></span></a> ';//저장하기 
					btnHtml+='<a href="javascript:;" onclick="viewlist();" class="bw-btn"><span><?=$txtdt["1087"]?></span></a> ';//목록 
					if(!isEmpty(obj["seq"]))
						btnHtml+='<a href="javascript:;" onclick="callapidel(\'recipe\',\'recipegoodsdelete\',\''+json+'\')" class="bdp-btn"><span><?=$txtdt["1154"]?></span></a>';//삭제하기 
				}
				else
				{
					btnHtml+='<a href="javascript:;" onclick="viewlist();" class="bw-btn"><span><?=$txtdt["1087"]?></span></a> ';//목록 
		...
		else if(obj["apiCode"]=="medicinetitle") //약재구성
		{
			...
			$(obj["sweet"]).each(function( index, value )
			{
				...
				if(layerpage == false)
				{
					...
					data+='	<td>'+value["rcMedititle"]+'</td>';  //청연약재명
					data+='	<td>'+value["rcOrigin"]+'/'+value["rcMaker"]+'</td>';  //청연 원산지/제조사
	...
	callapi('GET','recipe','recipegoodsdesc','<?=$apidata?>'); 	//실속처방  상세 API 호출
