...
{
	var packCnt=parseFloat($("input[name=odPackcnt]").val());
	var packCapa=parseFloat($("input[name=odPackcapa]").val());
	var sugarcapa=$("select[name=dcSugar]").children("option:selected").data("capa");
	console.log("getSugarCapa  packCnt = " + packCnt+", packCapa = " +packCapa+", sugarcapa = " + sugarcapa);
	var capa=getSugarTotalCapa(packCnt, packCapa, sugarcapa);
	return capa;
}
function getSugarTotalCapa(packCnt, packCapa, sugarcapa)
{
	console.log("getSugarCapa  packCnt = " + packCnt+", packCapa = " +packCapa+", sugarcapa = " + sugarcapa);
...
function resetamount()
{
	...
	dcWater=dooWater;
	console.log("########## dc_special = " + dc_special);
	...
	//-------------------------------------------------------------------------------
	console.log("dc_special = "+dc_special+", watertotal = "+watertotal+", dcTime = " + dcTime + ", packcnt = "+packcnt+", packcapa = "+packcapa+", dooWater = "+dooWater+", dcWater = " + dcWater+", dcAlcohol = " + dcAlcohol);
	...
	totalmediprice = medipricetotal + sweetpricetotal;
	...
	//감미제 
	//교이5bx,270,32,8640
	var dcSugarName=$("#dcSugar option:selected").val();
	var dcSugarCode=$("#dcSugar option:selected").data("code");
	var dcSugarCapa=$("#dcSugar option:selected").data("capa");
	var dcSugarPrice=$("#dcSugar option:selected").data("price");
	dcSugarPrice=(!isEmpty(dcSugarPrice)) ? parseFloat(dcSugarPrice) : 0;
	var dcSugarTotalCapa=getSugarCapa();
	var dcSugarAmount=parseInt(dcSugarPrice*dcSugarTotalCapa);

	//HD017603KR0004J,교이5bx,5,270,32
	if(!isEmpty(dcSugarCode))
	{
		var sugardata=dcSugarCode+","+dcSugarName+","+dcSugarCapa+","+dcSugarTotalCapa+","+dcSugarPrice;
		$("input[name=dcSugarData]").val(sugardata);
		amountdjmedi["sugar"]=dcSugarName+","+dcSugarTotalCapa+","+dcSugarPrice+","+dcSugarAmount;//감미제 
	}
	else
	{
		$("input[name=dcSugarData]").val("");
		amountdjmedi["sugar"]="";//감미제 
	}

	//특수탕전비 
	var specialName=$("#dcSpecial option:selected").text();
	var specialAmount=$("#dcSpecial option:selected").data("price");
	var specialval=$("#dcSpecial option:selected").val();
	specialAmount=!isEmpty(specialAmount)?specialAmount:0;
	var specialtotalamount=0;

	console.log("specialval = " + specialval);
	if(isEmpty(chkdcSpecial(specialval)))
	{
	...
	else
	{
		if(chkdcSpecial(specialval)=="alcohol")//주수상반
		{
			...
			specialtotalamount=calcolhol*specialAmount;//주수상반
		}
		else if(chkdcSpecial(specialval)=="distillation")//증류탕전
		{
			specialtotalamount=(packcnt*packcapa)*specialAmount;
		}
		...
		$("input[name=dcSpecialPrice]").val(specialAmount);
		console.log("@@@@ specialName = " + specialName+", specialAmount = " + specialAmount+", specialtotalamount = " + specialtotalamount);
		amountdjmedi["special"]=specialName+","+specialtotalamount;//특수탕전비 
	}
	...
	//주문금액
	//-------------------------------------------------------------------------------
	console.log("totalmediprice = "+totalmediprice+", decoctionprice = "+decoctionprice+", totalmaking = "+totalmaking+", packprice = "+packprice+", boxprice = "+boxprice+", dcSugarAmount = "+dcSugarAmount+", specialtotalamount = "+specialtotalamount);
	odAmount = totalmediprice + decoctionprice + totalmaking + packprice + boxprice + dcSugarAmount + specialtotalamount;
	//10원단위 
...
function setAmountDjmedi()
{
	...
	if(!isEmpty($("textarea[name=odAmountdjmedi]").val()))
	{
		...
		//감미제
		//{"medicine":"567.9,20,11358","sweet":"","totalmedicine":11358,"sugar":"교이5bx,270,32,8640","special":"주수상반(청주),1000","poutch":"0,1,0","medibox":"0,1,0","delibox":"0,1,0","packing":"5000,1,5000","dcshape":"0,1,0","totalpack":5000,"decoction":"17600,1,17600","makingprice":"150,45,6750","infirst":"0,1,0","inafter":"0,1,0","making":6750,"release":"4000,2,8000","totalamount":58340}
		//교이5bx,270,32,8640
		//console.log("sugar :: "+adjmedi["sugar"]);
		...
		if(!isEmpty(sugararr[0]))
		{
			...
			data="<div class='table_style'>";
			data+="<ul>";
			data+="<li class='left'>"+sugararr[0]+"</li><li>"+sugararr[1]+"g * "+commasFixed(sugararr[2])+getTxtData("UNIT")+" = "+commasFixed(sugararr[3])+getTxtData("UNIT")+"</li>";
			data+="</ul>";
			data+="</div>";
			$("#tot_sugarprice").html(data);
			var price=!isEmpty(sugararr[3])?sugararr[3]:0;
			if(isNaN(price)){price=0;}

			data=commasFixed(price)+getTxtData("UNIT");
			$("#tot_sugartotalprice").html(data);
		}
		else
		{
			$("#trsugar").hide();
		...
		//"특수탕전비 special":"주수상반(청주),1000"
		var specialarr=adjmedi["special"].split(",");
		console.log("specialspecialspecialspecial : "+adjmedi["special"]);
		if(!isEmpty(specialarr[0])&&parseInt(specialarr[1])>0)
		{
			$("#trspecial").show();
			data="<div class='table_style'>";
			data+="<ul>";
			data+="<li class='left'>"+specialarr[0]+"</li><li>"+commasFixed(specialarr[1])+getTxtData("UNIT")+"</li>";
			data+="</ul>";
			data+="</div>";
			$("#tot_specialprice").html(data);
			data=commasFixed(specialarr[1])+getTxtData("UNIT");
			$("#tot_specialtotalprice").html(data);
		}
		else
		{
			$("#trspecial").hide();
