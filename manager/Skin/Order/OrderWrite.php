...
//selstr='<select title="'+title+'" class="reqdata resetcode '+width+'" name="'+name+'" id="'+name+'" onchange="codeChange(this);" '+disable+' >';
	...
	function loadWInfoData()
	{
		...
		if(!isEmpty(obj["odGoods"]) && obj["odGoods"]=="M")//첩제(약재포장)
		{
			mCheck="checked";
		}
...
<script>
	...
	function changeMatype()
	{
		...
		if(chkMatype()==="pill") //제환
		{
			...
			$("#ordermatype").load("<?=$root?>/Skin/Order/OrderPill.php?seq="+seq+"&medical="+medical+"&rc_seq="+rc_seq+"&rc_type="+rc_type+"&pilltotalcapa="+pilltotalcapa+"&goods="+odGoods, function(){setAmountDjmedi();});
		...
		if(chkMatype()==="pill") //제환
		{
			...
			//$("#orderdecoction").load("<?=$root?>/Skin/Order/OrderPill.php?seq="+seq, function(){loadPillData();resetpill();});
			$("#ordermatype").load("<?=$root?>/Skin/Order/OrderPill.php?seq="+seq);
		}
		else //탕제 
		{
			$("#ordermatype").load("<?=$root?>/Skin/Order/OrderDecoction.php?goods="+odGoods, function(){loadDecoctionData();resetamount();resetmedi();});
		}
		*/
	}
	function pillkeyup()
	{
		...
		if(chkMatype()=="pill") //제환
		{
			...
			var rc_seq=$("input[name=rc_seq]").val();//rc_seq 
			var rc_type=$("input[name=rc_type]").val();//rc_type 
			var odPillcapa=$("input[name=odPillcapa]").val();
			var odQty=$("input[name=odQty]").val();
			var pilltotalcapa=0;
			...
			pilltotalcapa=odPillcapa*odQty;
			console.log("seq : " + seq+", medical :" +medical+", rc_seq : " + rc_seq+"&rc_type : " + rc_type+", pilltotalcapa = " + pilltotalcapa);
			$("#ordermatype").load("<?=$root?>/Skin/Order/OrderPill.php?seq="+seq+"&medical="+medical+"&rc_seq="+rc_seq+"&rc_type="+rc_type+"&pilltotalcapa="+pilltotalcapa+"&goods="+odGoods);
	...
	function callGInfo()
	{
		$("#orderinfo").load("<?=$root?>/Skin/Order/OrderGInfo.php", function(){loadGInfoData();callAmount();});
	}
	//수기처방 주문정보 
	function callWInfo()
	{
			...
			for(var key in poarr)
			{
				...
				if(!isEmpty(poarr[key]["order"]))
				{
					...
					case "juice"://착즙
						...
						break;
					case "mixed"://혼합
						console.log("혼합결합제량 : " + $("input[name=pillBindersliang]").val());
						...
						poarr[key]["order"][0]["plBindersliang"]=$("input[name=pillBindersliang]").val();
	...
	function makepage(json)
	{
		...
		/*else if(obj["apiCode"]=="medicalimg") //파우치, 한약박스, 포장박스, 제환, 항아리, 스틱 list 새로 받아서 다시 갱신하자 
		{
			//------------------------------------------
			// 포장 및 박스
			//------------------------------------------
			//탕제파우치
			//parsepackcodes("odPacktypeDecocDiv", obj["odPacktypeList"], '<?=$txtdt["1382"]?>', 'odPacktype', obj["odPacktype"], obj["odPackprice"],'<?=$txtdt["1235"]?>','');

			//한약박스종류선택
			//parsepackcodes("reBoxmediDiv", obj["reBoxmediList"], '<?=$txtdt["1468"]?>', 'reBoxmedi', obj["reBoxmedi"], obj["reBoxmediprice"],'<?=$txtdt["1235"]?>','');

			//배송포장재종류
			//parsepackcodes("reBoxdeliDiv", obj["reBoxdeliList"], '<?=$txtdt["1396"]?>', 'reBoxdeli', obj["reBoxdeli"],obj["reBoxdeliprice"], '<?=$txtdt["1235"]?>','');

			var type=$('input:radio[name="maType"]:checked').val();
			var capa="<?=$BASE_PACKCAPA?>";
			switch(type)
			{
			...
			//가격필드가 비어있으면 새로 뽑아온 데이터로 바뀐다
			$("input[name=odPackprice]").val("");


			console.log("medicalimgmedicalimgmedicalimgmedicalimg");
			resetamount();
		}*/		
		...
		else if(obj["apiCode"] == "smulist")//상시처방 리스트
		{
			var data = "";
