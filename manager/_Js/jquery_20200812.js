...
function saveupdate(result){
	...
	if(obj["resultCode"]=="200")
	{
		...
		else if(obj["apiCode"]=="policyupdate")
		{
			alertsign("success", obj["stateName"]+"되었습니다.", "", "1000");
			var url=obj["returnData"]+"?seq="+obj["poGroup"];
			goPolicyWrite(url);
		}
		else if(obj["apiCode"]=="policyorderchange")
		{
			alertsign("success", "순서를 변경하였습니다.", "", "1000");
			var url=obj["returnData"]+"?seq="+obj["poGroup"];
			goPolicyWrite(url);
		}	
		else
...
function goPolicyWrite(url)
{
	$("#listdiv").load(url);  //seq값이 있으면
}
function savedelete(result)
{
	...
	{
		if(obj["apiCode"]=="policydelete")
		{
			console.log("policydelete "+ obj["poGroup"]);
			alertsign("success", "삭제되었습니다.", "", "1000");
			var url=obj["returnData"]+"?seq="+obj["poGroup"];
			goPolicyWrite(url);
		}	
		else
...
function necdata()
{
	...
	$(".necdata").each(function()
	{
        ...
        if($(this).attr("name")=="mdWater" || $(this).attr("name")=="dcMillingloss" || $(this).attr("name")=="dcLossjewan" || $(this).attr("name")=="dcBindersliang" || $(this).attr("name")=="dcCompleteliang"|| $(this).attr("name")=="dcJungtang" || $(this).attr("name")=="dcRipen" || $(this).attr("name")=="dcDry" || $(this).attr("name")=="pbPrice" || $(this).attr("name")=="md_stable") //흡수율이 ㅇ 인 경우도 있어 예외처리
        {
            if(isEmpty($(this).val()))
            {
                if(data!=""){data+=",";}
...
function setDcWaterAlcohol(name, data)
{
	...
	if(name=="dcSpecial")
	{
		if(chkdcSpecial(data)=="alcohol")//주수상반
		{
			$("#speDiv").show();
		}
		else
...
function CodeRadio(pgid, list, title, name, classname, data, readonly)
{
    ...
    $.each(list, function(val)
    {
		...
		//if(name=="NYchk" && raValue=="M")  //팝업에서 팝업사용여부일때만
		//{
			//radiostr += '<input type="text"  name="myip" class="reqdata" title = "나의 ip" id="myip" placeholder="나의 ip를 입력해주세요">';
		//}
...
function viewdesc(seq){
	...
	var search=hdata[2];
	if(search ===undefined){search="";}
	console.log("viewdesc page="+page+", seq="+seq+", search="+search);
	makehash(page,seq,search)
}
...
function calcDcWater(dcTime, watertotal, packcnt, packcapa)
{
	...
	var dooWater=newWater + evaporation;

	//5000cc이하일 경우 증발량 900적용(기본이 700이여서 -200만 더함 )
	if(parseInt(dooWater)<=5000)
	{
...
{
	var evaporation=700;//증발량
	var water=watertotal+(od_packcapa*od_packcnt);
	return water;
}
