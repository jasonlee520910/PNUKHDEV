 ...
 * FILE : SEED128.php
 *
 * DESCRIPTION: modify source of biz seed  for PHP
 *
 * ADD FUNCTION :  padding
 *								 convertByteDimension
 *								 getBytes
 *								 byteArrayToHex
 *								 hexToByteArray
 *								 getEncryptData
...
// If endianness is not defined correctly, you must modify here.
 	...
 	//private $ENDIAN	 = BIG;				// Java virtual machine uses big endian as a default
...
/****************************** Common functions ******************************/
	...
	private function GetB0($A){	return 0x000000ff & $A; }
	private function GetB1($A){	return 0x000000ff & ($A>>8); }
	private function GetB2($A){	return 0x000000ff & ($A>>16); }
	private function GetB3($A){	return 0x000000ff & ($A>>24); }
	private function EndianChange($dws)   { return ($dws>>24) | ($dws<<24) | (($dws<<8)&0x00ff0000)|(($dws>>8)&0x0000ff00); }
...
// Copying output values from last round to outData
  	...
  	for ($i=0; $i<4; $i++)
  	{
	  	$outData[$i] = $this->ConvertByte(($R0>>(8*(3-$i)))&0xff);//((($R0)>>(8*(3-$i)))&0xff);
	  	$outData[4+$i] = $this->ConvertByte(($R1>>(8*(3-$i)))&0xff);//((($R1)>>(8*(3-$i)))&0xff);
	  	$outData[8+$i] = $this->ConvertByte(($L0>>(8*(3-$i)))&0xff);//((($L0)>>(8*(3-$i)))&0xff);
	  	$outData[12+$i] = $this->ConvertByte(($L1>>(8*(3-$i)))&0xff);//((($L1)>>(8*(3-$i)))&0xff);
	  }
  	  	return $outData;
	}
...
/************************ Functions for Key schedule ************************* orig
    ...
    $T00 = $this->ConvertInt($A) + $this->ConvertInt($C) - $this->ConvertInt($this->KC[$Z]);

    $T11 = $this->ConvertInt($B) + $this->ConvertInt($this->KC[$Z]) - $this->ConvertInt($D);  
...
/************************ Functions for Key schedule **************************/
    ...
    $A = ($A >> 8 & 0x00ffffff) ^ ($B << 24);                        
    $B = ($B >> 8 & 0x00ffffff) ^ ($T0 << 24);                       
    $T00 = $this->ConvertInt($A&0xFFFFFFFF) + $this->ConvertInt($C&0xFFFFFFFF) - $this->ConvertInt($this->KC[$Z]&0xFFFFFFFF);

    $T11 = $this->ConvertInt($B&0xFFFFFFFF) + $this->ConvertInt($this->KC[$Z]&0xFFFFFFFF) - $this->ConvertInt($D&0xFFFFFFFF);  
    $K[0] = $this->SS0[$this->GetB0($T00)] ^ $this->SS1[$this->GetB1($T00)] ^ $this->SS2[$this->GetB2($T00)] ^ $this->SS3[$this->GetB3($T00)];    
    $K[1] = $this->SS0[$this->GetB0($T11)] ^ $this->SS1[$this->GetB1($T11)] ^ $this->SS2[$this->GetB2($T11)] ^ $this->SS3[$this->GetB3($T11)];  
	}
...
// reorder for little endian	  
    ...
    $pdwRoundKey[$nCount++] = $K[0];	$pdwRoundKey[$nCount++] = $K[1];
    //array size is length*2, roundkey 0~43. real key length 22  so add while 
    while($nCount<44){
    $pdwRoundKey[$nCount++] = 0;
  	}    
        return $pdwRoundKey;
	}
	...
	private function ConvertInt($float) {
		...
		if(is_float($float) && $float < $UnsignedIntMin){
			...
			$n=($divisiion%2 == 0)?0:$UnsignedIntMin;
	...
	//convert to byte
	private function ConvertByte($integer) {
		$c=(($integer+128)%256)-128;
	    return $c;
	}	
	
	//padding
	private function padding($dataByte=array()) {

		$quotient = count($dataByte) / 16;
		$remain = count($dataByte) % 16;
		...
		for($i=0; $i<$loop; $i++) {
			...
			for($j=0; $j<16; $j++) {
				...
				if($point <= count($dataByte)-1) {
					 $result[$i][$j] = $dataByte[$point];
				} else {
					$result[$i][$j] = 0x00;
				}
			}
  ...
  // array byte 2Dimension->1Dimension 
	private function convertByteDimension($dataByte) {
		$result = array();
		
		$point=0;
		
		for($i=0; $i<count($dataByte); $i++) {
			for($j=0; $j<16; $j++) {
				$point = ($i * 16) + $j;
	...
	//getbytes function
	private function getBytes($gdata) {
		$result = array();
		
		for($i=0; $i<strlen($gdata); $i++) {
				$result[$i] = ord($gdata[$i]);
			}
		return $result;
	}
	...
	//Encrypt start
	public function getEncryptData($key, $data) {

		$pbUserKey=array();
		$dataByte=array();
		$outdata=array();
		$pbUserKey = SEED128::getBytes($key);
		$dataByte = SEED128::padding(SEED128::getBytes($data));

		//roundKey create
		$pdwRoundKey = SEED128::SeedRoundKey($pdwRoundKey,$pbUserKey);
		$resultByte = array();
		$resultBuf = array();

		for($i=0; $i<count($dataByte); $i++) {
			$resultBuf = SEED128::SeedEncrypt($dataByte[$i], $pdwRoundKey,$outData);
		...
		$result = SEED128::convertByteDimension($resultByte);

		return SEED128::byteArrayToHex($result);
	}
	
		//Decrypt start
	public function getDecryptData($key, $data) {
		$dataBuf=array();
		$pbUserKey=array();
		$dataByte=array();
		$dataBuf = SEED128::hexToByteArray($data);
		
		$pbUserKey = SEED128::getBytes($key);
		$dataByte = SEED128::padding($dataBuf);
		...
		$resultBuf=array();
		for($i=0; $i<count($dataByte); $i++) {
			$resultBuf = SEED128::SeedDecrypt($dataByte[$i], $pdwRoundKey,$outData);
			for($j=0; $j<16; $j++) {
				$resultByte[$i][$j] = $resultBuf[$j];
			}
		}
