...
<input type="hidden" name="apiCode" class="reqdata" value="commercialupdate">
<input type="hidden" name="returnData" class="reqdata" value="<?=$root?>/Skin/Recipebasic/CommercialList.php">
...
<script>
	...
	function parsepackcodes(pgid, list, name, data)
	{
		...
		for(var key in list)
		{
			...
			//20200420 : 상비는 차가 있기에 파우치없음이 있어야함.
			if(list[key]["pbCode"]=="RBM200117151838" || list[key]["pbCode"]=="PCB200117151735" )
			{
				///안보이게 처리함
				/*						
					RBM200117151838
	...
	function commercialupdate()
	{
		if(necdata()=="Y") //필수조건 체크
		{
			...
			callapi("POST","recipe","commercialupdate",jsondata);
		}
	}
    function makepage(json)
    {
		...
		if(obj["apiCode"] == 'commercialdesc')
		{
			...
			if(!isEmpty(obj["seq"]) && (obj["rcStatus"]=="F"))
			{
				if(Auth=="true")
				{
					btnHtml='<a href="javascript:;" onclick="commercialupdate();" class="bdp-btn"><span><?=$txtdt["1070"]?></span></a> ';//저장하기 
					btnHtml+='<a href="javascript:;" onclick="viewlist();" class="bw-btn"><span><?=$txtdt["1087"]?></span></a> ';//목록 
					if(!isEmpty(obj["seq"]))
						btnHtml+='<a href="javascript:;" onclick="callapidel(\'recipe\',\'commercialdelete\',\''+json+'\')" class="bdp-btn"><span><?=$txtdt["1154"]?></span></a>';//삭제하기 
				}
				else
				{
					btnHtml+='<a href="javascript:;" onclick="viewlist();" class="bw-btn"><span><?=$txtdt["1087"]?></span></a> ';//목록 
	...
	callapi('GET','recipe','commercialdesc','<?=$apidata?>'); 	//실속처방  상세 API 호출
