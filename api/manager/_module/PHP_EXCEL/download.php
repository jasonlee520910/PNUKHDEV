<?php
	...
	$headers = array('동','호수','동호수','핸드폰번호1','핸드폰번호2','핸드폰번호3','핸드폰번호4');
	$rows = array();
	...
	$res=dbqry($sql);
	while($dt=dbarr($res)){
		$rowdata=array($dt["HOUSE_ID_DONG"],$dt["HOUSE_ID_HO"],$dt["HOUSE_NAME"],$dt["MOBILE_NO"],$dt["MOBILE_NO2"],$dt["MOBILE_NO3"],$dt["MOBILE_NO4"]);
		array_push($rows, $rowdata);
	}
	$data = array_merge(array($headers), $rows);
	 
	// 스타일 지정
	$widths = array(10, 10, 20, 23, 23, 23, 23);
	...
	$excel->getActiveSheet()->getStyle("A1:G".(count($rows)+1))->applyFromArray($fontStyle);
	...
	header("Content-Disposition: attachment; filename=kumho_".date("Ymdhis").".xlsx");
