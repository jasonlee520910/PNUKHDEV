...
<div id="pagecode" value="myrecipelist"></div>
...
<!-- s : 게시판 페이징 -->
<div class='' id="myrecipelistpage"></div>
<!-- e : 게시판 페이징 -->
...
<script>
		...
		if(obj["apiCode"]=="myrecipelist")
		{
			...
			if(!isEmpty(obj["list"]))
			{
				$(obj["list"]).each(function( index, value )
				{
					...
					data+="<td class='l'>"+value["rcTitle"]+"</td>"; //처방명 
			...
			//페이지
			getsubpage("myrecipelistpage",obj["tpage"], obj["page"], obj["block"], obj["psize"]);
		}
		return false;
	}
	...
	callapi('GET','recipe','myrecipelist',apidata);  
	$("#searchTxt").focus();
</script>
