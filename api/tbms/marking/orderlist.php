<?php  //marking  orderlist
	...
	else
	{
		...
		$wsql.=" and (d.mr_staffid='' or d.mr_staffid is null or d.mr_staffid='".$staffid."') ";
		if($process)
		{
			$arr=explode(",",$process);
			$prosql=" and ( ";
		...
		$pg=apipaging("od_seq","order",$jsql,$wsql);

		$osql=" group by a.od_seq, to_char(a.od_date, 'yyyy-mm-dd hh24:mi:ss'), a.od_code, a.od_title, a.od_use, a.od_status,a.od_name, a.od_userid, m.mi_name, e.re_name  ";
		$osql.=" order by decode(a.od_status,'marking_processing',1,'marking_start',2,'marking_apply',3,'marking_stop',4,'marking_cancel',5),to_char(a.od_date, 'yyyy-mm-dd hh24:mi:ss') desc  ";
		...
		$sql=" select * from ( ";
		$sql.=" select ROW_NUMBER() OVER (order by decode(a.od_status,'order','paid','marking_processing','marking_start','marking_apply','marking_stop','marking_cancel','release_apply','release_start','release_processing','release_stop','release_cancel','done'),to_char(a.od_date, 'yyyy-mm-dd hh24:mi:ss') desc) NUM , to_char(a.od_date, 'yyyy-mm-dd hh24:mi:ss') as ODDATE, a.od_code, a.od_title, a.od_use, a.od_status,a.od_name, a.od_seq, a.od_userid,   m.mi_name, e.re_name ";
