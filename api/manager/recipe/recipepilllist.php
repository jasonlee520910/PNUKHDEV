<?php  
	...
	$page=$_GET["page"];
	if($apicode!="recipepilllist"){$json["resultMessage"]="API(apiCode) ERROR_uniquesclist";$apicode="recipepilllist";}
	...
	else
	{
		...
		$jsql.="left join ".$dbH."_goods g on a.RC_CODE = g.gd_recipe  ";
		$wsql=" where a.rc_use in ('F','Y') and  a.rc_medical='pill' and g.GD_USE in ('A','Y')    ";  ///약속 goods
		...
		$sql.=" ,a.rc_seq, a.rc_code as rcCode, a.rc_source, a.rc_sourcetit, a.rc_title_kor as rcTitle,a.rc_medicine as RCMEDICINE";
		...
		$sql.=" ,g.GD_PILLORDER , g.GD_BOMCODE";
		...
		while($dt=dbarr($res))
		{
			...
			$rcMedicineName = getPillTitle($dt["GD_BOMCODE"]); ///han_medicine_djmedi mmtitle 가져오기 
			///------------------------------------------------------------
			...
			$arr=explode(",",$rcMedicineName);///약재갯수(약미)
			$addarray = array(
				"seq"=>$dt["RC_SEQ"], 
				"rcCode"=>$dt["RCCODE"], 
				"matching"=>$matching,
