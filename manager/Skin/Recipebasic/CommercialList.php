...
<div id="pagecode" value="commerciallist"></div>
...
<!-- s : 게시판 페이징 -->
<div class='' id="commerciallistpage"></div>
<!-- e : 게시판 페이징 -->
...
<script>
		...
		if(obj["apiCode"]=="commerciallist")
		{
			...
			if(!isEmpty(obj["list"]))
			{
				$(obj["list"]).each(function( index, value )
				{
					...
					data+="<td>"+value["rcPackType"]+"</td>"; // 파우치
					data+="<td>"+value["rcMediBox"]+"</td>"; // 한약박스
			...
			//페이지
			getsubpage("commerciallistpage",obj["tpage"], obj["page"], obj["block"], obj["psize"]);
		}
		return false;
	}
	...
	callapi('GET','recipe','commerciallist',apidata);  
	$("#searchTxt").focus();
</script>
