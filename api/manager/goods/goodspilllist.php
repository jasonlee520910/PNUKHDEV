<?php
	...
	$page=$_GET["page"];
	if($apiCode!="goodspilllist"){$json["resultMessage"]="API(apiCode) ERROR";$apiCode="goodspilllist";}
	...
	else
	{
		...
		$search="";
		$wsql="  where a.od_use in ('Y','C') and a.od_goods='G' and a.OD_MATYPE='pill' "; //사전조제이면서 제환인것만 
		...
		$jsql.=" inner join ".$dbH."_member h on a.od_staff=h.me_userid ";
		$jsql.=" left join ".$dbH."_order_client oc on a.od_keycode=oc.keycode ";
		$jsql.=" inner join ".$dbH."_pill p on a.od_keycode=p.PL_KEYCODE ";
		...
		//------------------------------------------------------------
		$hCodeList = getNewCodeTitle("odStatus,pillOrder");
		//------------------------------------------------------------
		...
		while($dt=dbarr($res))
		{
			...
			//------------------------------------------------------------
			$machinestat=explode("_",$dt["PL_MACHINESTAT"]);
			$pl_machinestat=$machinestat[0];

			$odStatus=$dt["OD_STATUS"];
			$odStatus = getodStatus($odStatusList, $odStatus, $pillOrderList, $pl_machinestat);
			//------------------------------------------------------------
	...
	//				  $status -> 주문상태 값
	//=========================================================================
	function getodStatus($list, $status, $mlist, $mstatus)
	{
		...
		{
			if($list[$i]["cdCode"] == $substat)
			{
				$statName = $list[$i]["cdName"];
			}
			if($list[$i]["cdCode"] == $substat2)
			{
				if($substat2 == 'stop')
				{
					$statName2="<span style='color:purple;'>".$list[$i]["cdName"]."</span>";
				}
				else if($substat2 == 'cancel')
				{
					$statName2="<span style='color:red;'>".$list[$i]["cdName"]."</span>";
				}
				else
				{
					$statName2 = $list[$i]["cdName"];
				}
			}
		}
		...
		{
			if($mlist[$i]["cdCode"] == $mstatus)
			{
				$statName4 = $mlist[$i]["cdName"];
				break;
			}
		}
		...
		if($statName4)
		{
			return $statName." ".$statName2."(".$statName4.")";
		}
