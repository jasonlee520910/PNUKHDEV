...
<div id="pagecode" value="smulist"></div>
...
<script>
		...
		if(obj["apiCode"]=="uniquesclist")
		{
			...
			if(!isEmpty(obj["list"]))
			{
				$(obj["list"]).each(function( index, value )
				{
					...
					data+="<td><a href='javascript:;'>"+value["rcTitle"]+"</a></td>"; //처방명 
			...
			//페이지
			getsubpage("smulistpage",obj["tpage"], obj["page"], obj["block"], obj["psize"]);
		}
		return false;
	}

	//처방서적>상시처방 리스트 API 호출
	...
	callapi('GET','recipe','uniquesclist',apidata);  //고유처방
	$("#searchTxt").focus();
</script>
