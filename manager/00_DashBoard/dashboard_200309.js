...
function searchsendapi(seardata)
{
	...
	{
		data = "searyear="+searyear+"&searmonth="+searmonth+"&searday="+searday+"&seartime="+seartime+"&searyear1="+searyear1+"&searmonth1="+searmonth1+"&searday1="+searday1+"&seartime1="+seartime1+searchtxt+seardata;
	}
	else
	{
...
//약재일보고서 > 년 선택시 --------------------
$(".searyearsel").focus(function(){
	selpreviousday=$(this).val();
}).change(function(){
	$(this).blur();
	medisearchyear("start", selpreviousday);
});
//약재보고서 > 년을 선택시 
$(".searyearsel1").focus(function(){
	selpreviousday=$(this).val();
}).change(function(){
	$(this).blur();
	medisearchyear("end", selpreviousday);
});
function medisearchyear(type, selpreviousday)
{
	var today = new Date();
	...
	var searyear1=$("select[name=searyear1]").val();//두번째년
	console.log("searyearsel  selyy = " + selyy +", yyyy =" + yyyy);
	$("input[name=searchtxt]").val("");

	if(parseInt(selyy)>parseInt(searyear1))
	{
		alert("연도를 다시 선택해 주세요."+selpreviousday);
		if(type=="end")
		{
			$("#searyear1").val(selpreviousday).attr("selected", "selected");
		}
		else
		{
			$("#searyear").val(selpreviousday).attr("selected", "selected");
		}		
		return;
	}
	...
	if(selyy <= yyyy)
	{
		...
		*/
		...
		for(i=1;i<=month;i++)
		{		
			$("#searmonth").append("<option value='"+i+"'>"+i+"월</option>");
		}
		searchsendapi("");
	}
}
...
//약재일보고서 > 월 선택시 --------------------
$(".searmonthsel").focus(function(){
	selpreviousday=$(this).val();
}).change(function(){
	$(this).blur();
	medisearchmonth("start", selpreviousday);
});
//약재보고서 > 월을 선택시 
$(".searmonthsel1").focus(function(){
	selpreviousday=$(this).val();
}).change(function(){
	$(this).blur();
	medisearchmonth("end", selpreviousday);
});
function medisearchmonth(type, selpreviousday)
{
	var today = new Date();
	var dd = today.getDate();
	var mm = today.getMonth()+1; //January is 0!
	var yyyy = today.getFullYear();
	var selyy=$("select[name=searyear]").val();//첫번째년
	var selmm=$("select[name=searmonth]").val(); //첫번째월 

	var searyear1=$("select[name=searyear1]").val();//두번째년
	var searmonth1=$("select[name=searmonth1]").val();//두번째월
	console.log("searmonthsel  selmm = " + selmm +", mm =" + mm+", searyear1 = "+searyear1+", searmonth1 = " + searmonth1);
	$("input[name=searchtxt]").val("");

	if(parseInt(selyy)>parseInt(searyear1))
	{
		alert("연도를 다시 선택해 주세요.");
		$("#searyear1").val(selyy).attr("selected", "selected");
		return;
	}

	if(parseInt(selyy)==parseInt(searyear1) && parseInt(selmm) >parseInt(searmonth1))
	{
		alert("월을 다시 선택해 주세요.");
		if(type=="end")
		{
			$("#searmonth1").val(selpreviousday).attr("selected", "selected");
		}
		else
		{
			$("#searmonth").val(selpreviousday).attr("selected", "selected");
		}
		return;
	}
	...
	else
	{
		if(parseInt(mm) >= parseInt(selmm))
		{
			...
			{
				day=pad(i,2);
				$("#searday").append("<option value='"+day+"'>"+i+"일</option>");
			}

			//해당년월의 일 추가 
			var lastdate1=new Date(searyear1, searmonth1, 0);
			var lastday1 = lastdate.getDate();
			var day1="";

			if(parseInt(searmonth1)==parseInt(mm))
				lastday1=dd;

			for(i=1;i<=lastday1;i++)
			{
				day1=pad(i,2);
				$("#searday1").append("<option value='"+day1+"'>"+i+"일</option>");
			}
...
$(".seardaysel").focus(function(){
	selpreviousday=$(this).val();
}).change(function(){
	$(this).blur();
	medisearchday("start", selpreviousday);
});
$(".seardaysel1").focus(function(){
	selpreviousday=$(this).val();
}).change(function(){
	$(this).blur();

	medisearchday("end", selpreviousday);

});
function medisearchday(type, selpreviousday)
{
	var selyy=$("select[name=searyear]").val();//첫번째년
	var selmm=$("select[name=searmonth]").val(); //첫번째월 
	var seldd=$("select[name=searday]").val(); //첫번째일

	var searyear1=$("select[name=searyear1]").val();//두번째년
	var searmonth1=$("select[name=searmonth1]").val();//두번째월
	var searday1=$("select[name=searday1]").val(); //두번째일

	if(parseInt(selyy)>parseInt(searyear1))
	{
		alert("연도를 다시 선택해 주세요. ");
		$("#searyear").val(selyy).attr("selected", "selected");
		return;
	}
	...
	{
		alert("월을 다시 선택해 주세요. ");
		$("#searmonth").val(selmm).attr("selected", "selected");
		return;
	}

	if(parseInt(selyy)==parseInt(searyear1) && parseInt(selmm) == parseInt(searmonth1) && parseInt(seldd)> parseInt(searday1))
	{
		alert("일을 다시 선택해 주세요. ");
		if(type=="end")
		{
			$("#searday1").val(selpreviousday).attr("selected", "selected");
		}
		else
		{
			$("#searday").val(selpreviousday).attr("selected", "selected");
		}
		return;
	}
...
/*
//약재보고서 > 일을 선택시 
$(".seardaysel").on("focus",function(){
	selpreviousday=$(this).val();
	alert(selpreviousday);
	}).("change",function(){

	alert("change"+selpreviousday);
	var selyy=$("select[name=searyear]").val();//첫번째년
	var selmm=$("select[name=searmonth]").val(); //첫번째월 
	var seldd=$("select[name=searday]").val(); //첫번째일
	...
	{
		alert("월을 다시 선택해 주세요.");
		$("#searmonth1").val(selmm).attr("selected", "selected");
		return;
	}
	...
	{
		alert("일을 다시 선택해 주세요.");
		//$("#searday1").val(seldd).attr("selected", "selected");
		return;
	}
...
*/
//약재보고서 > 일을 선택시 
$(".seardaysel1").on("change",function(){
...
$(".seartimesel").on("change",function(){
	$("input[name=searchtxt]").val("");

	searchsendapi("");
});

$(".seartimesel1").on("change",function(){
	$("input[name=searchtxt]").val("");
