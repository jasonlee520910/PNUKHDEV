<?php  
	///처방관리 > 상비처방 > 상비처방 등록&수정
	$apicode=$_POST["apiCode"];
	$language=$_POST["language"];
	$rc_code=$_POST["rcCode"];
	if($apicode!="commercialupdate"){$json["resultMessage"]="API(apiCode) ERROR";$apicode="commercialupdate";}
	...
	else
	{
		...
		else
		{
			...
			//처방 입력시 goods도 등록
			$sql2=" insert into ".$dbH."_goods (gd_seq,gd_recipe, gd_loss, gd_losscapa, gd_capa,gd_stable,gd_type,gd_code,gd_unit,gd_name_kor,gd_desc,gd_use,gd_date) values ((SELECT NVL(MAX(gd_seq),0)+1 FROM ".$dbH."_goods),'".$rc_code."','0','0','1','1000','commercial','".$gd_code."','1','".$rc_title."','".$gd_desc."','Y',SYSDATE) ";  
			dbcommit($sql2);
		}
