	...
	*/
	...
	$("input[name=search]").val(json["search"]);
		...
		if(json["resultCode"]=="200")
		{
			...
			$.each(json["list"] ,function(index, val){
				...
				else if(json["apiCode"]=="patientlist" || json["apiCode"]=="medicalrecordlist")
				{
					data+="<tr>";				
				}
				else
				...
				for(i=0;i<marr.length;i++)
				{
					...
					else if(marr[i]=="ordertype" || marr[i]=="rcType" ||  marr[i]=="rcMedicineCnt" ||  marr[i]=="rcChub" ||  marr[i]=="rcPackcnt" ||  marr[i]=="rcPackcapa")
					{
						var txt=val[marr[i]];
						cls="td-txtcenter";
					}
	...
	{		
		...
		$("#layermedicalbox").load(url,function(){callapi("GET","/medical/patient/",getdata("medicalrecordlist")+"&userid="+userid+"&name="+name);  });
	}
